...
export class CalendarController {
  ...
  @ApiResponse({ status: 201, description: 'Calendar slot created successfully', type: CalendarSlotResponse })
  async create(@Body() createCalendarSlotDto: CreateCalendarSlotDto, @Request() req) {
    return this.calendarService.create(createCalendarSlotDto, req.user.id);
  }

  @Get()
  @ApiOperation({ summary: 'Get all calendar slots' })
  @ApiResponse({ status: 200, description: 'List of calendar slots', type: [CalendarSlotResponse] })
  ...
  @ApiResponse({ status: 200, description: 'Calendar slot details', type: CalendarSlotResponse })
  ...
  @ApiResponse({ status: 200, description: 'Calendar slot updated successfully', type: CalendarSlotResponse })
  @ApiResponse({ status: 404, description: 'Calendar slot not found' })
  async update(@Param('id') id: string, @Body() updateCalendarSlotDto: UpdateCalendarSlotDto, @Request() req) {
